...
/**** Student tables ****/
    ...
    create table student_address (
        ...
        state_cd varchar(2),
        zip_cd varchar(5),
        primary key (student_id, address_type_cd)
    );
    ...
    alter table student_address 
        add constraint FK_3dv0a77trvmgdsjapk3iy1ops 
        foreign key (state_cd) 
        references state;
    alter table student_address 
        add constraint FK_mwifv4t2o9e79h4t5ljfdp463 
        foreign key (student_id) 
        references student;
    alter table student_course 
        add constraint FK_6fqmc7x848099n8nc7vtv6pfi 
        foreign key (course_id) 
        references course;
    alter table student_course 
        add constraint FK_1gx1u34qc1nxy52fwsqf5fpc2 
        foreign key (student_id) 
        references student;

    create view person_delivery_address as 
       select p.person_id, p.first_name, p.last_name, p.mar_status_cd,
              pa.street_line1_tx, pa.street_line2_tx, pa.city_tx, 
              pa.state_ab, pa.zip_cd, pa.zip_sfx_cd,
              pa.cntry_cd, pa.fips_cd, pa.latitude, pa.longitude
       from person p
       inner join person_address pa on pa.person_id = p.person_id
       where pa.addr_usg_cd = 'D';
